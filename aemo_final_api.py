...
logging.basicConfig(
    ...
    stream=sys.stdout,   # explicit stdout so log output is visible in terminal
)
...
COMMON_HEADERS = {
    ...
    "X-market":                  "NEM",
     "Accept":                    "text/xml; aseXML_version=r46",
    "System":                    "MSATS/CATS",
...
# Static fallback: common AEMO network tariff code → description.
# Used when the r44 XML strips NetworkAdditionalInformation from registers.
NETWORK_TARIFF_DESCRIPTIONS = {
    # ── Endeavour Energy (NSW) ─────────────────────
    "N70":   "N70 Domestic Network Charges",
    "N71":   "TOU Seasonal Interval",
    "N50":   "Controlled Load 1",
    "N51":   "Controlled Load 2",
    "N61":   "TOU Generation Interval",
    # ── Ausgrid (NSW) ──────────────────────────────
    "EA010": "Domestic All Time",
    "EA025": "TOU Residential",
    ...
    "N24":   "TOU Residential",
    # ── AusNet Services (VIC) ──────────────────────
    "NEE11": "Residential Single Rate",
    "NEE12": "Residential TOU",
    "NEE13": "Controlled Load",
    "NEE14": "Dedicated Circuit",
    # ── CitiPower / Powercor (VIC) ─────────────────
    "A100":  "Residential Single Rate",
    "A200":  "TOU Residential",
    "A300":  "Controlled Load",
    # ── SA Power Networks ──────────────────────────
    "AGSLD": "General Supply Single Rate",
    "ATSLD": "TOU Residential",
    "ADCLD": "Controlled Load",
    # ── Energex (QLD) ──────────────────────────────
    "8400":  "Residential Flat",
    "8900":  "Controlled Load",
    "8500":  "TOU Residential",
}
...
DISCOVERY_PARAM_SPEC = [
    ...
    ("houseNumberSuffix",       "optional",    "House number suffix"),
...
def validate_discovery_params(body: dict) -> list:
    # Auto-split alphanumeric house numbers (e.g., '16A' -> '16', 'A')
    if "houseNumber" in body and isinstance(body["houseNumber"], str):
        m = re.match(r"^(\d+)\s*([a-zA-Z])$", body["houseNumber"].strip())
        if m:
            body["houseNumber"] = m.group(1)
...
def find_in(el, *tags):
    ...
    Scoping search to a block prevents bleeding between NMIs.
    """
...
def parse_discovery_xml(xml_text):
    ...
    for block in root.iter():
        ...
        nmi_parent = next(
            (p for p in block.iter() if any(c is nmi_el for c in p)),
            block
        )
...
def parse_detail_xml(xml_text):
    ...
    FIX: Meter/register status check uses `is not None` guard so that <Meter>
    blocks with no <Status> child (register-only split-meter blocks) are NOT
    skipped — ensuring NetworkAdditionalInformation and all register fields
    ...
    for meter_el in block.iter():
        ...
        # FIX: `is not None` — <Meter> blocks with no <Status> child must NOT
        # be skipped; they carry the RegisterConfiguration with all register data
        # including NetworkAdditionalInformation.
        if meter_status is not None and meter_status not in ACTIVE_STATUSES:
        ...
        if reg_config is not None:
            for reg_el in reg_config.iter():
                ...
                if reg_id:
                    ...
                    logger.debug(
                        "[parse_detail] register=%s  tariff=%s  additionalInfo=%s",
                        reg_id,
                        reg_fields.get("NetworkTariffCode"),
                        reg_fields.get("NetworkAdditionalInformation"),
                    )
    ...
    # ── Fill missing networkAdditionalInfo ────────────────────────────────────
    # Pass 1: scan ALL <Register> elements in the XML to build tariff → info map
    tariff_info_lookup = {}
    for any_reg in block.iter():
        if local_tag(any_reg.tag) != "Register":
            continue
        tc_el  = None
        nai_el = None
        for ch in any_reg:
            lt = local_tag(ch.tag)
            if lt == "NetworkTariffCode":
                tc_el = ch.text.strip() if ch.text else None
            if lt == "NetworkAdditionalInformation":
                nai_el = ch.text.strip() if ch.text else None
        if tc_el and nai_el and tc_el not in tariff_info_lookup:
            tariff_info_lookup[tc_el] = nai_el

    # Pass 2: fill from XML lookup, then static fallback map
    for r in all_registers:
        if not r.get("networkAdditionalInfo"):
            tc   = r.get("tariffCode")
            info = tariff_info_lookup.get(tc) or NETWORK_TARIFF_DESCRIPTIONS.get(tc)
            if info:
                r["networkAdditionalInfo"] = info
                logger.debug("[parse_detail] filled networkAdditionalInfo for %s  tariff=%s → %s",
                             r["registerId"], tc, info)
    ...
    for attempt in range(1, 4):
        try:
            ...
            if resp.status_code == 400:
                return resp, 400, f"AEMO rejected request (400): {resp.text[:1000]}"
...
def nmi_lookup():
    ...
    Step 1: NMIDiscovery  — returns ALL NMIs at the address
    ...
    if len(nmi_list) > 5:
        msg = "Please enter a more specific address."
        audit_record(
            "step1_NMIDiscovery_limit_exceeded",
            transaction_id, discovery_params,
            f"{BASE_URL}/NMIDiscovery", d_status,
            {"nmisFound": len(nmi_list)}, msg, step1_ms
        )
        return jsonify({
    ...
    def fetch_detail(entry: dict) -> dict:
        try:
            ...
            # ── Log raw XML to terminal via logger (safe on Windows) ──────────
            # NOTE: print(..., flush=True) raises OSError on Windows when Flask
            # replaces sys.stdout with a non-standard stream. Use logger instead.
            if dr is not None:
                logger.info(
                    "[fetch_detail] nmi=%s  http_status=%s\n%s\n%s\n%s",
                    nmi, d2_status, "=" * 80, dr.text, "=" * 80,
                )
            else:
                logger.warning("[fetch_detail] nmi=%s  dr=None  err=%s", nmi, d2_err)
            # ─────────────────────────────────────────────────────────────────
            ...
            # Prefer richer address from getNMIDetail; fall back to discovery address
            ...
            # Catch-all: use logger.error (not print) to avoid Windows OSError 22
            logger.error(
                "[fetch_detail] UNCAUGHT EXCEPTION nmi=%s  error=%s",
                entry.get("nmi"), exc, exc_info=True,
            )
...
def nmi_status():
    """
    POST /api/nmi-status
    Required: nmi (10-char alphanumeric), checksum (single digit)
